...
public class MainScreen {
    ...
    public static HBox build() {
        ...
        HBox root=new HBox(); root.setPrefSize(1280,760); root.setStyle("-fx-background-color:#FAF8F6;");
        topBar=new TopBar("Dashboard"); contentArea=new StackPane(); contentArea.setStyle("-fx-background-color:#FAF8F6;");
        ScrollPane scroll=new ScrollPane(contentArea); scroll.setFitToWidth(true); scroll.setFitToHeight(true); scroll.setStyle("-fx-background-color:#FAF8F6;-fx-background:#FAF8F6;"); HBox.setHgrow(scroll,Priority.ALWAYS);
        VBox right=new VBox(topBar,scroll); VBox.setVgrow(scroll,Priority.ALWAYS); right.setStyle("-fx-background-color:#FAF8F6;"); HBox.setHgrow(right,Priority.ALWAYS);
        root.getChildren().addAll(new com.gestorcerveja.ui.components.Sidebar(MainScreen::navigateTo),right);
        navigateTo("dashboard"); return root;
    }

    public static void navigateTo(String id) {
        String title=switch(id){
            case "dashboard"->"Dashboard";case "pedidos"->"Pedidos";case "clientes"->"Clientes";
            case "faturas"->"Faturas";case "receitas"->"Receitas";case "lotes"->"Lotes";
            case "producao"->"Etapas de Produção";case "ingredientes"->"Ingredientes";
            case "veiculos"->"Veículos";case "utilizadores"->"Utilizadores";
            case "qualidade"->"Testes de Qualidade";case "stock"->"Movimentações de Stock";
            case "perfil"->"O Meu Perfil";default->id;
        };
        ...
        ScreenBundle bundle=switch(id){
            ...
            case "pedidos"->PedidosScreen.build();case "clientes"->ClientesScreen.build();
            case "faturas"->FaturasScreen.build();case "receitas"->ReceitasScreen.build();
            case "lotes"->LotesScreen.build();case "producao"->ProducaoScreen.build();
            case "ingredientes"->IngredientesScreen.build();case "veiculos"->VeiculosScreen.build();
        ...
        if(topBar!=null){
            ...
            if(bundle.table()!=null){final String t=title;topBar.setOnExport(()->ExportUtils.export(bundle.table(),contentArea.getScene().getWindow(),t));}
            else topBar.setOnExport(null);
        }
    }
}
