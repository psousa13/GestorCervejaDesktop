...
public class ReceitasScreen {
    public static ScreenBundle build() {
        TableView<String[]> table=TableBuilder.buildTable(new String[]{"ID","Nome","Descrição","Preço/L"},new String[0][]);
        Runnable refresh=()->{
            try {
                table.setItems(FXCollections.observableArrayList(MainScreen.receitaController.listAll().stream().map(r->{
                    String preco; try{preco=String.format("€ %.2f/L",MainScreen.receitaController.getActivePrice(r.getId()));}catch(Exception ex){preco="Sem preço";}
                    return new String[]{String.valueOf(r.getId()),r.getNome(),r.getDescricao()!=null?r.getDescricao():"—",preco};
                }).toList()));
        ...
        table.getColumns().add(TableActions.column(row->openEditModal(row,refresh),
            row->{try{MainScreen.receitaController.delete(Integer.parseInt(row[0]));refresh.run();}catch(Exception e){throw new RuntimeException(e);}}));
        ...
        return new ScreenBundle(root,table,()->new ModalOverlay(MainScreen.contentArea,"Nova Receita",fields,values->{
            String nome=values.get("nome"); if(nome.isBlank()) throw new IllegalArgumentException("Nome obrigatório.");
            MainScreen.receitaController.create(nome,values.get("descricao"));
            String ps=values.get("preco"); if(!ps.isBlank()){double p=Double.parseDouble(ps.replace(',','.'));var l=MainScreen.receitaController.listAll();if(!l.isEmpty())MainScreen.receitaController.setPrice(l.get(l.size()-1).getId(),p);}
            refresh.run();
        }).show());
    }
    private static void openEditModal(String[] row,Runnable refresh){
        ...
        new ModalOverlay(MainScreen.contentArea,"Editar Receita #"+id,fields,values->{
            ...
            String ps=values.get("preco"); if(!ps.isBlank()) MainScreen.receitaController.setPrice(id,Double.parseDouble(ps.replace(',','.')));
            refresh.run();
        }).show();
    }
}
