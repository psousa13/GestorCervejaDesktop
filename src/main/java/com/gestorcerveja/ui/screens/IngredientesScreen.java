...
public class IngredientesScreen {
    public static ScreenBundle build() {
        TableView<String[]> table = TableBuilder.buildTable(
                new String[]{"ID","Nome","Unidade","Stock Atual","Stock Mín.","Estado"}, new String[0][]);
        Runnable refresh = () -> {
            try {
        ...
        table.getColumns().add(TableActions.column(
            ...
            row -> { try { MainScreen.ingredienteController.delete(Integer.parseInt(row[0])); refresh.run(); } catch (Exception e) { throw new RuntimeException(e); } }
        ));
        ...
        return new ScreenBundle(root, table, () -> new ModalOverlay(MainScreen.contentArea,"Novo Ingrediente",fields,values -> {
            String nome=values.get("nome"); if(nome.isBlank()) throw new IllegalArgumentException("Nome obrigatório.");
            MainScreen.ingredienteController.create(nome,values.get("unidade"),pd(values.get("stockAtual")),pd(values.get("stockMinimo"))); refresh.run();
        }).show());
    }
    private static void openEditModal(String[] row, Runnable refresh) {
        ...
        new ModalOverlay(MainScreen.contentArea,"Editar Ingrediente",fields,values -> {
            MainScreen.ingredienteController.update(id,values.get("nome"),values.get("unidade"),pd(values.get("stockAtual")),pd(values.get("stockMinimo"))); refresh.run();
        }).show();
    }
