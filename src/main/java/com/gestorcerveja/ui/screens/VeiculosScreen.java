...
public class VeiculosScreen {
    public static ScreenBundle build() {
        TableView<String[]> table=TableBuilder.buildTable(new String[]{"ID","Matrícula","Nome","Tipo","Marca","Capacidade","Ocupação"},new String[0][]);
        Runnable refresh=()->{
            try{table.setItems(FXCollections.observableArrayList(MainScreen.veiculoController.listAll().stream().map(v->new String[]{
                String.valueOf(v.getId()),v.getMatricula(),v.getNome(),v.getTipo(),v.getMarca(),v.getCapacidade()+"L",v.getOcupacaoAtual()+"L / "+v.getCapacidade()+"L"
        ...
        table.getColumns().add(TableActions.column(row->openEditModal(row,refresh),
            row->{try{MainScreen.veiculoController.delete(Integer.parseInt(row[0]));refresh.run();}catch(Exception e){throw new RuntimeException(e);}}));
        ...
        List<FormField> newFields=List.of(
            ...
            new FormField("tipo","Tipo",FormField.Type.COMBO,List.of("Tanque","Barril","Cisterna","Camião","Outro")),
        ...
        return new ScreenBundle(root,table,()->new ModalOverlay(MainScreen.contentArea,"Novo Veículo",newFields,values->{
            String mat=values.get("matricula"); if(mat.isBlank()) throw new IllegalArgumentException("Matrícula obrigatória.");
            MainScreen.veiculoController.create(mat,values.get("marca"),values.get("cor"),values.get("nome"),pd(values.get("capacidade")),values.get("tipo")); refresh.run();
        }).show());
    }
    private static void openEditModal(String[] row,Runnable refresh){
        ...
        new ModalOverlay(MainScreen.contentArea,"Editar Veículo #"+id,fields,values->{
            MainScreen.veiculoController.update(id,values.get("nome"),values.get("tipo"),values.get("marca"),values.get("cor"),pd(values.get("capacidade"))); refresh.run();
        }).show();
    }
